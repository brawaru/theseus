...
import { createTypedFetch, type TypedFetch } from "../../typed-fetch/index.ts";
...
export function createAPI({ apiBaseURL, fetchOpts }: APIConfig) {
  const $apiFetch = createTypedFetch<paths>({
    ...fetchOpts,
    baseURL: new URL("v2/", apiBaseURL).toString(),
  });
  ...
  return {
    ...
    methods: {
      ...
      getAllSessions(options?: FetchOverrides) {
        return $apiFetch.base<Session[]>("/session/list", options);
      },
      refreshSession(options?: FetchOverrides) {
        return $apiFetch.base<NewSession>("/session/refresh", {
          method: "post",
          ...options,
        });
      },
      deleteSession(id: string, options?: FetchOverrides) {
        return $apiFetch.base(joinPathParts("/session", id), {
          method: "delete",
          ...options,
        });
      },
      getUser(idOrUsername: string, options?: FetchOverrides) {
        return $apiFetch("/user/{id|username}", {
